...
openssl req -new -x509 -days 365 -key ca.key \
  -subj "/C=DE/CN=cert-injector-webhook"\
  -out ca.crt
...
openssl x509 -req \
  -extfile <(printf "subjectAltName=DNS:cert-injector-webhook.cert-injector.svc") \
  -days 365 \
  -in server.csr \
...
kubectl create secret tls cert-injector-webhook-tls \
  --namespace cert-injector \
  --cert=server.crt \
  --key=server.key \
